...
combat_mat_03 <- read.table("180-day spaceflight_M180-1/RNA-seq/nobatch_expression_profile.txt", header=T, row.names=1, as.is=T, sep="\t")
SampleSheet_03 <- read.csv("180-day spaceflight_M180-1/RNA-seq/SampleSheet.csv")
...
#T1_to_T2_DEG_table.txt, T1_to_T2_DEG_table.txt, T1_to_T3_DEG_table.txt and DEG_list.Rds are available in "result"


#DEG trend in T1_to_T2 and T2_to_T3 (M90)
DEG_list_01 <- readRDS(file = "result/90-day spaceflight_M90/DEG_list.Rds")
...
saveRDS(DEG_list_trends, file = "result/90-day spaceflight_M90/DEG_list_trends.Rds")

#DEG trend in T1_to_T2 and T2_to_T3 (M180-1)
DEG_list_02 <- readRDS(file = "result/180-day spaceflight_M180-1/DEG_list.Rds")
trends <- t(apply(state_groups, 1, function(x){return(c(state[x]))}))
trends <- as.data.frame(trends)
colnames(trends) <- c("T1_to_T2","T2_to_T3")
DEG_list_trends <- apply(trends, 1, function(x){
...
DEG_list_trends <- DEG_list_trends[-grep("stable_stable", names(DEG_list_trends))]
DEG_list_trends <- DEG_list_trends[mapply(length, DEG_list_trends )!=0]
saveRDS(DEG_list_trends, file = "result/180-day spaceflight_M180-1/DEG_list_trends.Rds")

#DEG trend in T1_to_T2 and T2_to_T3 (M180-2)
DEG_list_03 <- readRDS(file = "result/180-day spaceflight_M180-2/DEG_list.Rds")
trends <- t(apply(state_groups, 1, function(x){return(c(state[x]))}))
trends <- as.data.frame(trends)
colnames(trends) <- c("T1_to_T2","T2_to_T3")
DEG_list_trends <- apply(trends, 1, function(x){
...
DEG_list_trends <- DEG_list_trends[-grep("stable_stable", names(DEG_list_trends))]
DEG_list_trends <- DEG_list_trends[mapply(length, DEG_list_trends )!=0]
saveRDS(DEG_list_trends, file = "result/180-day spaceflight_M180-2/DEG_list_trends.Rds")
